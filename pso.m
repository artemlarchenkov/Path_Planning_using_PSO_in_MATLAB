%
% Авторские права (c) 2015, Yarpiz (www.yarpiz.com)
% Все права защищены. Пожалуйста, прочтите "license.txt" для условий лицензии.
%
% Код проекта: YPAP115
% Заголовок проекта: Планирование пути с использованием PSO в MATLAB
% Издатель: Yarpiz (www.yarpiz.com)
% 
% Разработчик: С. Мостафа Калами Херис (Член команды Yarpiz)
% 
% Контактная информация: sm.kalami@gmail.com, info@yarpiz.com
%
...
function offspring = Crossover(parents, pc)
    ...
    offspring = repmat(empty_particle, nParents, 1);
    ...
    for i = 1:2:nParents
        ...
        if rand() <= pc
            ...
            % Создаем потомков
            offspring(i).Position.x = [parent1.Position.x(1:crossoverPoint), parent2.Position.x(crossoverPoint + 1:end)];
            offspring(i).Position.y = [parent1.Position.y(1:crossoverPoint), parent2.Position.y(crossoverPoint + 1:end)];

            offspring(i + 1).Position.x = [parent2.Position.x(1:crossoverPoint), parent1.Position.x(crossoverPoint + 1:end)];
            offspring(i + 1).Position.y = [parent2.Position.y(1:crossoverPoint), parent1.Position.y(crossoverPoint + 1:end)];
        else
            % Если скрещивание не происходит, потомки равны родителям
            offspring(i) = parent1;
